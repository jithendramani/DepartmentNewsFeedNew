...
public FormatDescription(text: string):string{
    var returnString:string="";
    if(text!=null&&text.length>0){
      returnString =  text.replace(/(<([^>]+)>)/ig,"").split("&#160;").join(" ");
      returnString = returnString.replace(/\s/g," ");
    }
if(returnString.length>=130){
    returnString =returnString.substring(0, 127) + "...";
}
    return returnString;

}

public trimText(text:string,length: number):string{
...
public formatModifiedDate(modDate:Date):string{
    var monthArr =["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
    var month = monthArr[modDate.getMonth()];
   return month +" "+modDate.getDate()+", "+modDate.getFullYear();
}
